	...
	function allch()
	{
		for(i=0;i<<%=pagelist3.size()%>;i++)
		{
		  ...
		  <TR align="center" bgColor=#ffffff>
			...
			<TD id=map><%
			String str="普通会员，享受9.8折优惠";int num=cb.getCount("select jf from member where username='"+pagelist2.get(1).toString()+"'");if(num>=5000)str="银卡会员，享受9.5折优惠"; 
			else if(num>=12000)str="金卡会员，享受9.8折优惠"; 
	...
	var pageCount=parseInt(form3.pageCount.value);
  	...
  	if(form3.busjump.value>pageCount){//如果跳转文本框中输入的页数超过最后一页的数，则跳到最后一页
		if(pageCount==0){	
			...
			var url = "<%=basePath%><%=dir %>/member/person.jsp?page=1";
			form3.action=url;
			form3.submit();
		}
		else{
			var url = "<%=basePath%><%=dir %>/member/person.jsp?page="+pageCount;
			form3.action=url;
			form3.submit();
		}	
	}
